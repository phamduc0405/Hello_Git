...
# Lấy List coin trong Top 20 Volumn(USDT) 
# Đầu tư 5 coin với số tiền ảo 10000$
# Check khi có tín hiệu mua sẽ mua vào khi len data_Buy <5 (mỗi lần mua sẽ add vào 1 data_Buy(Time, Tên, Số Tiền))
# Khi Len data_Buy > 0 , get name in data_Buy and check for Sell (Mỗi lần sell sẽ del row)
class MyMoney:
    wallet = 10000
    buy_price = 0
    log_buy = 0
...
def getminutedata(sym,inter,lookback1,lookback2):   
   ...
   # rame = client.get_historical_trades(sym)
   # print(rame)
    frame =frame.iloc[:,:8]
    frame.columns = ['Time','Open','High','Low','Close','Volume','Close_Time','Usd_Volume']
    frame = frame.set_index('Time')
    frame.index = pd.to_datetime(frame.index, unit ='ms')
    frame = frame.astype(float)
    #frame['test']=frame['Volume']*frame['Close']
    return frame
...
#--------------------------------------------------------------------------Get List USDT In Margin -------------------------------------------------
def get_20usdtvolume(filter='USDT'):
    rows = []
    ...
    for i in range (0,(int(len(unwanted)))):  
        if unwanted[i] in rows :
            rows.remove(unwanted[i])
    return rows

#------------------------------------------------------------------------------- USDT & Top 20 (Market Cap ) ------------------------------------------
def get_20marketcap(filter='USDT'):
    rows = []
    ...
    rows = pd.DataFrame(rows,columns=['s','name','cs','c','cap','qv'])
    rows['s']=info.s
    rows['name']=info.b
    rows['cs']=info.cs
    rows['c']=info.c
    ...
    rows = rows.nlargest(20, ['qv']).reset_index() 
    rows.sort_values('qv')
    return rows
...
#--------------------------------------------------------------------------------------- Main Cycle ------------------------------------------------
def volume20_main():                 #--------------------- Lấy Dữ Top 20 USDT Volume
    Listcoin=get_20usdtvolume() 
    update_data = Crawl_Data('BTCUSDT')
    ...
    for i in range(20,len(update_data)) :
        ...
        print(len(Listcoin))
        for a in range(0,len(Listcoin)) :
            curr_data = Crawl_Data(Listcoin[a])           
            if  ((curr_data.iloc[i-2]['CCI']<100 < curr_data.iloc[i]['CCI'] or curr_data.iloc[i-2]['RSI']<80 < curr_data.iloc[i]['RSI'] )and 
                 curr_data.iloc[i-1]['in_uptrend10'] == curr_data.iloc[i-1]['in_uptrend20'] == True) :  
                 buy = True
            else : buy = False                    
            rows.append([Listcoin[a],Crawl_Data(Listcoin[a]).Usd_Volume[i],buy])
              
        df = pd.DataFrame(rows,columns=['Symbol','Usd_Volume','Buy'])
        print('--------')
        print(df)     
        # for i in range(20,len(update_data)):                 
